...
export const ItemPlanner = () => {
  ...
  // Calculate combined materials cost
  const combinedMaterials = {};
  if (baseItem && baseItem.materials) {
    Object.entries(baseItem.materials).forEach(([material, quantity]) => {
      combinedMaterials[material] = (combinedMaterials[material] || 0) + quantity;
    });
  }
  if (prefix && prefix.materials) {
    Object.entries(prefix.materials).forEach(([material, quantity]) => {
      combinedMaterials[material] = (combinedMaterials[material] || 0) + quantity;
    });
  }
  if (suffix && suffix.materials) {
    Object.entries(suffix.materials).forEach(([material, quantity]) => {
      combinedMaterials[material] = (combinedMaterials[material] || 0) + quantity;
    });
  }
        ...
        <div>
          ...
          {Object.keys(combinedMaterials).length > 0 && (
            <div style={{ marginTop: '40px' }}>
              <h3>Materials Cost (still formula how these are calculated is missing)</h3>
              <div style={{ 
                display: 'flex', 
                gap: '15px', 
                flexWrap: 'wrap',
                ...
                {Object.entries(combinedMaterials)
                  .sort(([, a], [, b]) => b - a)
